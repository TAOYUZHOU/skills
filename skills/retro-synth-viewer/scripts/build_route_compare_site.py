...
from v4_local import route_logS_from_steps, try_membership_batch, try_path_local_str_audits  # noqa: E402
...
def existence_of(smi: str, memb: Dict[str, dict]) -> str:
    info = memb.get(smi) or {}
...
def terminal_class(smi: str, memb: Dict[str, dict]) -> str:
    info = memb.get(smi) or {}
    if info.get("in_reaction_dataset"):
        return "strong"
    if info.get("in_pubchem"):
...
def dump_to_path(obj: dict, a_of: Optional[dict] = None) -> list:
    ...
    for node in walk(obj):
        ...
        raw = rxn_of(node)
        if a_of and raw in a_of:
            vt.update(a_of[raw] or {})
        a = vt.get("A")
        steps.append(
            SimpleNamespace(
...
def _leaf(smi: str, svg_cache: dict, memb: dict) -> dict:
    return {
        "kind": "leaf",
        "smiles": smi,
...
def convert_node(node: dict, depth: int, svg_cache: dict, memb: dict) -> dict:
    smi = target_of(node)
    rxn = rxn_of(node)
    kids = children_of(node)
    if ">>" not in rxn:
        return _leaf(smi, svg_cache, memb)

    child_nodes = [convert_node(ch, depth + 1, svg_cache, memb) for ch in kids]
    shown = {c.get("smiles") for c in child_nodes}
    leftover = [r for r in reactants_of(node) if r and r not in shown]
    if not child_nodes:
        child_nodes = [_leaf(r, svg_cache, memb) for r in reactants_of(node)]
    else:
        child_nodes.extend(_leaf(r, svg_cache, memb) for r in leftover)
...
def convert_route(
    ...
    *,
    ...
    tree = convert_node(obj, 0, svg_cache, memb)
...
def build_from_manifest(manifest: dict, *, base: Path, a_of: Optional[dict], memb: Optional[dict]) -> dict:
    ...
    for case, raw_routes in loaded:
        ...
        first = next(iter(raw_routes.values()), ({}, {}))[1]
...
def main() -> None:
    ...
    if args.payload:
        ...
        payload = build_from_manifest(manifest, base=man_path.parent, a_of=a_of, memb=memb)
    else:
