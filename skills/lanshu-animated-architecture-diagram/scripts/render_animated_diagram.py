...
_RESAMPLE_BICUBIC = getattr(getattr(Image, "Resampling", Image), "BICUBIC", Image.BICUBIC)
...
def render_static(spec):
    ...
    return ex, img.resize((width, height), _RESAMPLE_LANCZOS).convert("RGB")
...
def premium_finish(base):
    ...
    mask = mask_small.resize((width, height), _RESAMPLE_BICUBIC)
